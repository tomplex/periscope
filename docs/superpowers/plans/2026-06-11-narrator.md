...
def tick(panes: list[tuple[dict, dict]]) -> None:
    ...
    for w, _parsed in panes:
        ...
        try:
            ...
            row = activity.get_pane_status(pane_id)
...
Expected: the three new tests fail — `AttributeError: <module 'periscope.routes.pane'> does not have the attribute 'stamp_pane_rename'` (mocker.patch on a missing name), same for auto_rename.
