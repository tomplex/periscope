...
| `periscope/tmux_persist.py` | Provisions the tmux side of continuation-over-reboot (TPM + resurrect + continuum, owned `tmux.conf` sourced from `~/.tmux.conf`) + the `resurrect` healthz status |
